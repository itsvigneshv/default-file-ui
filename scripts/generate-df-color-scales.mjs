 ...
 *
 * Always emits both infix palettes:
 *   --df-{family}-detailed-{step}
 *   --df-{family}-compact-{step}
 * Plus unscoped aliases --df-{family}-{step} that follow data-df-color-scale.
 *
...
/** Emit --df-{family}-{density}-{step}: value */
...
/** Emit unscoped aliases --df-{family}-{step}: var(--df-{family}-{density}-{step}) */
...
function block(selector, bodyLines, comment) {
...
// Colored ramps use 25 as their base tint instead of white (0).
const STEP_NAMES = (() => {
...
/*
 * Primitive scales use infix density markers so both palettes always coexist:
 *   --df-{family}-detailed-{step}
 *   --df-{family}-compact-{step}
 * Unscoped aliases --df-{family}-{step} follow <html data-df-color-scale="compact|detailed">
 * (default: detailed).
 */

/* Both palettes always defined */
${block(
  ":root",
  [
  ...
  "Infix primitives: detailed + compact"
)}

/* Unscoped aliases (default = detailed) */
${block(
