 ...
 *
 * Theme tables live in df-theme.mjs (Tailwind v4 parity, DF-owned).
...
function declsFor(utility) {
  ...
  if (m) {
    ...
    if (arb) {
      const val = decodeArbitrary(arb[1])
      // Match Tailwind: text-[11px] → font-size; text-[#fff] → color
      if (
        val.startsWith("#") ||
...
css += `/* Theme: scripts/df-theme.mjs (Tailwind v4 parity → DF tokens / --df-neutral-*) */\n\n`
css += baseRules.join("\n\n") + "\n"
// Ascending breakpoint order so larger screens override smaller ones (Tailwind cascade)
