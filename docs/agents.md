...
The bundled `design-file-ui` skill routes marketing vs workspace vs mobileTool surfaces, and includes `references/kit.md` for discover then compose with these CLI and MCP tools. When the kit is present, agents must maximize registry components (especially on single-shot builds), style with kit color tokens (not Tailwind), and report any custom components that were not found.
